...
`;
        ...
        console.warn('⚠️ Gemini extraction error, falling back to heuristic extraction:', (err as Error).message);
      }
    }
...
`;
        ...
        console.warn('⚠️ Gemini field extraction failed, using heuristic parser:', (err as Error).message);
      }
    }
    ...
    if (gstMatch) fields.gstin = gstMatch[0];
...
`;
        ...
        console.warn('⚠️ Gemini verification failed, using heuristic evaluation:', (err as Error).message);
      }
    }
    ...
    let matchingDoc = vendorDocs.find(d => {
      if (reqCategory === 'Legal' && (d.documentType === 'GST Certificate' || d.documentType === 'Company Registration')) return true;
      if (reqCategory === 'Tax' && (d.documentType === 'PAN Card' || d.documentType === 'Income Tax Document')) return true;
      if (reqCategory === 'Financial' && d.documentType === 'Financial Statement') return true;
      if (reqCategory === 'Experience' && d.documentType === 'Experience Certificate') return true;
      if (reqCategory === 'Eligibility' && d.documentType === 'MSME/Udyam Certificate') return true;
      return false;
    });
...
`;
        ...
        console.warn('⚠️ Gemini risk analysis failed, using heuristic engine:', (err as Error).message);
      }
    }
    ...
    if (!docTypes.has('GST Certificate') && !vendor.gstNumber) missingDocs.push('GST Certificate');
    if (!docTypes.has('PAN Card') && !vendor.panNumber) missingDocs.push('PAN Card');
    if (!docTypes.has('Financial Statement')) missingDocs.push('Audited Financial Statements (Last 3 Years)');
    ...
    let riskScore = 15;
    if (missingDocs.length > 0) riskScore += missingDocs.length * 15;
    if (inconsistencies.length > 0) riskScore += inconsistencies.length * 25;
...
`;
        ...
        console.warn('⚠️ Gemini chat failed, using fallback copilot:', (err as Error).message);
      }
    }
